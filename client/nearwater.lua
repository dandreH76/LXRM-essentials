...
function WashPrompt()
    Citizen.CreateThread(function()
        ...
        PromptSetControlAction(WashPrompt, LXFramework.Shared.Keybinds['ENTER'])
...
function DrinkPrompt()
    Citizen.CreateThread(function()
        ...
        PromptSetControlAction(DrinkPrompt, LXFramework.Shared.Keybinds['J'])
        str = CreateVarString(10, 'LITERAL_STRING', str)
                ...
                if IsPedOnFoot(playerPed) then
                    if IsEntityInWater(playerPed) then
                        ...
                        if PromptHasHoldModeCompleted(DrinkPrompt) then
                            TriggerEvent('LXRM-river:client:drink')    
                        end
                    end
                end
            end
...
-- drink action
AddEventHandler('LXRM-river:client:drink', function()
    local src = source
    if drink ~= 0 then
    ...
    Citizen.Wait(17000)
    TriggerServerEvent("LXFramework:Server:SetMetaData", "thirst", LXFramework.Functions.GetPlayerData().metadata["thirst"] + math.random(50, 100))
