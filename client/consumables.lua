...
AddEventHandler('LXRM-essentials:client:DrinkingAnimation', function()
...
RegisterNetEvent("consumables:client:Drink", function(itemName)
    ...
    TriggerEvent('LXRM-essentials:client:DrinkingAnimation')
    ...
    LXFramework.Functions.Progressbar("drinking-water", "Drinking from the Flask", 4500, false, true,
    {
        ...
        TriggerServerEvent("LXFramework:Server:SetMetaData", "thirst", LXFramework.Functions.GetPlayerData().metadata["thirst"] + ConsumeablesDrink[itemName].thirst)
        TriggerServerEvent("LXFramework:Server:SetMetaData", "metaweight", LXFramework.Functions.GetPlayerData().metadata["metaweight"] + ConsumeablesDrink[itemName].metaweight)

        LXFramework.Functions.Notify("You drank from your Flask!", 'success', 3000)
    end)
end)
...
RegisterNetEvent("consumables:client:coffee", function(itemName)
    if isBusy then
        ...
        TriggerServerEvent("LXFramework:Server:SetMetaData", "thirst", LXFramework.Functions.GetPlayerData().metadata["thirst"] + ConsumeablesDrink[itemName])
        isBusy = not isBusy
    end
end)
...
RegisterNetEvent('consumables:client:UseFieldBandage', function()
    local ped = PlayerPedId()
    local hasItem = LXFramework.Functions.HasItem('fieldbandage', 1)
    if hasItem then
        LXFramework.Functions.Progressbar("use_fieldbandage", "Using Field Bandage..", 4000, false, true, {
            ...
            StopAnimTask(ped, "mini_games@story@mob4@heal_jules@bandage@arthur", "bandage_fast", 1.0)
            LXFramework.Functions.Notify(Lang:t('error.canceled'), "error")
        end)
    else
        LXFramework.Functions.Notify('you don\'t have any field bandages!', 'error')
    end
end)
...
RegisterNetEvent("consumables:client:Smoke", function(itemName)
    if isBusy then
        ...
        Wait(sleep)
        TriggerEvent("inventory:client:ItemBox", LXFramework.Shared.Items[itemName], "remove")
...
RegisterNetEvent("consumables:client:Eat", function(itemName)
    if isBusy then
        ...
        Wait(sleep)
        TriggerEvent("inventory:client:ItemBox", LXFramework.Shared.Items[itemName], "remove")
        TriggerServerEvent("LXFramework:Server:SetMetaData", "hunger", LXFramework.Functions.GetPlayerData().metadata["hunger"] + ConsumeablesEat[itemName].hunger)
        TriggerServerEvent("LXFramework:Server:SetMetaData", "metaweight", LXFramework.Functions.GetPlayerData().metadata["metaweight"] + ConsumeablesEat[itemName].metaweight)
        TriggerServerEvent('hud:server:RelieveStress', math.random(2, 4))
        ClearPedTasks(PlayerPedId())
        isBusy = not isBusy
        LXFramework.Functions.Notify("You Eating", 'success', 3000)
    end
end)
...
RegisterNetEvent("consumables:client:EatStew", function(itemName)
    if isBusy then
        ...
        TriggerServerEvent("LXFramework:Server:SetMetaData", "hunger", LXFramework.Functions.GetPlayerData().metadata["hunger"] + ConsumeablesEat[itemName])
        TriggerServerEvent("LXFramework:Server:SetMetaData", "metaweight", LXFramework.Functions.GetPlayerData().metadata["metaweight"] + ConsumeablesEat[itemName].metaweight)
        isBusy = not isBusy
    end
end)
